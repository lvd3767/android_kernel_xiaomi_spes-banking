...
static __maybe_unused struct dentry *nomount_hijacked_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags);
static __maybe_unused int nomount_hijacked_iterate_dir(struct file *file, struct dir_context *ctx);
static __maybe_unused void nomount_hijack_dentry_ops(struct dentry *dentry);
static __maybe_unused void nm_free_rule(struct nomount_rule *rule);
...
static __always_inline struct nomount_rule *nm_tree_search_path(u32 hash, u16 len, const char *path)
{
    ...
    while (node) {
        ...
        cmp = (hash != r->v_hash) ? (hash < r->v_hash ? -1 : 1) :
...
static __always_inline void nm_tree_insert(struct nomount_rule *new_rule)
{
    ...
    while (*link) {
        parent = *link;
        r = rb_entry(parent, struct nomount_rule, rb_node);
        cmp = (new_rule->v_hash != r->v_hash) ? (new_rule->v_hash < r->v_hash ? -1 : 1) :
